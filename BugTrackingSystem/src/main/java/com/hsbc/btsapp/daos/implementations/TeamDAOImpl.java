...
public class TeamDAOImpl implements TeamDAO {
	...
	public void addTeam(int pmId) throws CapacityException {
		...
		int team_count = getTeamCountOfmanager(pmId);
		if (team_count == 4) {
			throw new CapacityException("No more teams can be created. Max 4 allowed");
		}
		try {
			PreparedStatement pst1 = ConnectionUtils.getConnection()
					.prepareStatement("insert into Teams(user_id) values(?)");
			pst1.setInt(1, pmId);
	...
	public void updateTeam(Team team) throws TeamNotFoundException {
		...
		try {
			...
			if (!rs.next()) {
				...
				PreparedStatement pst_1 = conn.prepareStatement("update Teams set userId=?" + " where teamId=?");
	...
	public List<String> getTeamByTeamId(int teamId) throws TeamNotFoundException {
		...
		String query = "select userId ,userName from User where userId IN (select userId from User_Team_Mapping where teamId="
				+ teamId;
		try {
			Connection conn = ConnectionUtils.getConnection();
	...
	public int getTeamCountOfmanager(int pmId) {
		String sql = "select count(team_id) from Teams where user_id=?";
		PreparedStatement pst1;
		int teamCount = 0;
		try {
			pst1 = ConnectionUtils.getConnection().prepareStatement(sql);
			pst1.setInt(1, pmId);
			ResultSet rs = pst1.executeQuery();
			if (rs.next()) {
