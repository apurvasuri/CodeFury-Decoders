...
public class UserDAOImpl implements UserDAO {
	...
	public void addUser(User user) throws UserAlreadyExistsException {
		...
		try
		{
			...
			{
				PreparedStatement pst2 = conn.prepareStatement("insert into User(user_id, user_name, user_email, user_password, user_type) value(?,?,?,?,?)");
	...
	public void updateUser(User user) {
		...
		try {
			...
			{
				PreparedStatement pst2 = conn.prepareStatement("update User set user_name=?,user_email=?,user_password=?,user_type=? where user_id=?");
	...
	public User getUserById(int userId) {
		...
		try 
		{
			...
			if (rs.next()) 
			{
				user = new User(rs.getInt("user_id"), rs.getString("user_name"), rs.getString("user_email"),rs.getString("user_password"), UserTypes.valueOf(rs.getString("user_type")));
			} 
			else
			{
	...
	public List<User> getAllUsers() {
		...
		try
		{
			...
			while(rs.next())
			{
				userList.add(new User(rs.getInt("user_id"), rs.getString("user_name"), rs.getString("user_email"),rs.getString("user_password"), UserTypes.valueOf(rs.getString("user_type"))));
			} 
		} 
		catch (SQLException e) 
		{
